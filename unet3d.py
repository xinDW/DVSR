...
from model import conv3d, batch_norm, concat
from custom import LReluLayer, ReluLayer, SubpixelConv3d
...
def upconv3d(layer, out_channels, factor=2, mode='subpixel', act=tf.identity, name='upconv'):  
    with tf.variable_scope(name):
        if mode == 'subpixel':
            ...
            n = conv3d(layer, act=tf.identity, shape=[1,1,1, in_channels, out_channels*(factor**3)], strides=[1,1,1,1,1])
            n = SubpixelConv3d(n, scale=factor, n_out_channel=None, act=act, name=mode)
            return n
...
def unet3d(LR, ngf=64, reuse=False, is_train=False, name='unet3d'):
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        n = conv3d(n, shape=(4,4,4,in_channels,ngf), strides=(1, 1, 1, 1, 1), name='conv0')
        ...
        for out_channels in layer_specs:
            with tf.variable_scope('encoder_%d' % (len(layers) + 1)):
                ...
                conv = conv3d(rect, shape=(4, 4, 4, in_channels, out_channels), strides=(1,2,2,2,1))
